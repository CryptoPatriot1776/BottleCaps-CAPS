...
#include "capsaddressvalidator.h"
#include "capsunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Caps address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new CapsAddressEntryValidator(parent));
    widget->setCheckValidator(new CapsAddressCheckValidator(parent));
}
...
bool parseCapsURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no caps: URI
    if(!uri.isValid() || uri.scheme() != QString("caps"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CapsUnits::parse(CapsUnits::RVN, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCapsURI(QString uri, SendCoinsRecipient *out)
{
    // Convert caps:// to caps:
    //
    //    Cannot handle this later, because caps:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("caps://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "caps:");
    }
    QUrl uriInstance(uri);
    return parseCapsURI(uriInstance, out);
}

QString formatCapsURI(const SendCoinsRecipient &info)
{
    QString ret = QString("caps:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CapsUnits::format(CapsUnits::RVN, info.amount, false, CapsUnits::separatorNever));
        paramCount++;
    }
...
bool openCapsConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(CAPS_CONF_FILENAME);
    ...
    /* Open caps.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Caps.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Caps (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Caps (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "caps.desktop";
    return GetAutostartDir() / strprintf("caps-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a caps.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Caps\n";
        else
            optionFile << strprintf("Name=Caps (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the caps app
...
bool GetStartOnSystemStartup()
{
    CFURLRef capsAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (capsAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, capsAppUrl);

    CFRelease(capsAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add caps app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, capsAppUrl, nullptr, nullptr);
    }
