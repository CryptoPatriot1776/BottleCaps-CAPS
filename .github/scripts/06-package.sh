...
cd ${GITHUB_WORKSPACE}
PKGVERSION=`grep "PACKAGE_VERSION" src/config/caps-config.h | cut -d\" -f2`
VERSION="${PKGVERSION}"
...
if [[ ${GITHUB_BASE_REF} =~ "release" ]]; then
    DISTNAME="caps-${VERSION}"
else
    DISTNAME="caps-${VERSION}-${SHORTHASH}"
fi
...
if [[ ${OS} == "windows" ]]; then
    ...
    for rmfile in detach-sig-create.sh win-codesign.cert caps-cli.exe caps-qt.exe capsd.exe; do
        if [[ -e ${rmfile} ]]; then
            rm -f ${rmfile}
        fi
    done
    ...
    if [[ -e ${GITHUB_WORKSPACE}/dist/Caps-Qt.app/Contents/MacOS/install_cli.sh ]]; then
        chmod +x ${GITHUB_WORKSPACE}/dist/Caps-Qt.app/Contents/MacOS/install_cli.sh
    fi
    ...
    ${GITHUB_WORKSPACE}/depends/x86_64-apple-darwin14/native/bin/dmg dmg "Caps-Core.dmg" ${RELEASE_LOCATION}/${DISTNAME}-osx-unsigned.dmg
    ...
    if [[ -e ${RELEASE_LOCATION} ]]; then
        ...
        cd ${STAGE_DIR}
        cp -Rf ${DISTNAME}/bin/capsd .
        cp -Rf ${DISTNAME}/bin/caps-cli .
    else
        echo "release directory doesn't exist"
    fi
