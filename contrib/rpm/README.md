...
The RPM spec file provided here is for Caps-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://caps.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Caps git repository](https://github.com/CapsProject/Ravencoin) but are not part of
...
Currently only `Source10` is used, for the example `caps.conf` file.
...
in the caps git repository. At present only an SVG version of the Caps
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the caps
...
needed when 0.12.1 is released, a proper fix is already in the Caps git
master branch.
...
idea to build Ravencoin Core against a static version of that library compiled
...
The `caps.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `caps-server` package. At this point, most RPM based
...
The `caps-server` package should have SELinux support. How to properly do
...
Caps-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/caps/). He has been
packaging Caps for Fedora at least since 2012.
...
4. I renamed the `caps` package that contains the Qt GUI to `caps-core` as
that appears to be how the general population refers to it, in contrast to
`caps-xt` or `caps-classic`. I wanted to make sure the general population
