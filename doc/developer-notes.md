...
Run with the -testnet option to run with "play capss" on the test network, if you
...
Ravencoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- CapsMiner : Generates capss (if wallet is enabled).
...
However, in open source software such as Ravencoin Core, where everyone uses
...
pay attention to for reviewers of Ravencoin Core code.

General Ravencoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Ravencoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:caps/caps.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `caps-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `caps-cli` and the GUI debug console use this table to determine how to
