...
The Ravencoin Core daemon can be configured to act as a trusted "border
router", implementing the caps wire protocol and relay, making
...
The ZeroMQ feature in Ravencoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building capsd:
...
For instance:
    ...
    $ capsd -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/capsd.tx.raw
...
These options can also be provided in caps.conf.
...
From the perspective of capsd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into capsd directly. Furthermore, no information is
...
using. Capsd appends an up-counting sequence number to each
