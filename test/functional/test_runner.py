...
`test/functional/test_framework/test_framework.py:CapsTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/caps_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and capsd
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_capsd = config["components"].getboolean("ENABLE_CAPSD")
    if not (enable_wallet and enable_cli and enable_capsd):
        print("No functional tests to run. Wallet, utils, and capsd must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if capsd is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "capsd"]) is not None:
            print("%sWARNING!%s There is already a capsd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "CAPSD" not in os.environ:
        os.environ["CAPSD"] = build_dir + '/src/capsd' + exeext
        os.environ["CAPSCLI"] = build_dir + '/src/caps-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `caps-cli help` (`rpc_interface.txt`).
